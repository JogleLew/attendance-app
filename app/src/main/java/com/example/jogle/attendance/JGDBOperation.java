...
public class JGDBOperation {
	...
	public List<JGDataSet> getByDateAndUid(String date, int uid) {
		List<JGDataSet> list = null;
		SQLiteDatabase db = database.getReadableDatabase();
		String sql = "select * from record where time like ? and user_id like ?";
		String[] params = new String[]{date+"%", String.valueOf(uid)};
		Cursor cursor = db.rawQuery(sql, params);
