...
public abstract class KLMainActivity extends Activity {
    public List<KLBean> mDatas = new ArrayList<KLBean>();
    public List<KLBean> mBack = new ArrayList<KLBean>();;
...
//        initsearchview();
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.klmenu_add, menu);
        return true;
    }
    @Override
...
//            builder.append(line);
//        }
        String strUTF8 = URLDecoder.decode(test.toString(), "UTF-8");
        ...
        for(int k = 0;k<userarraynew.length();k++)
        {
            ...
            mDatas.add(new KLBean(bumen.getInt("id"), 0, bumen.getString("name")));
            mBack.add(new KLBean(bumen.getInt("id"), 0, bumen.getString("name")));
        ...
        for (int j = 0; j < array2.length(); j++) {
            ...
            mDatas.add(new KLBean( Integer.parseInt(ggg[0])+10000, Integer.parseInt(ggg[1]), user.getString("name")));
            mBack.add(new KLBean(Integer.parseInt(ggg[0])+10000,Integer.parseInt(ggg[1]), user.getString("name")));
        ...
        try {
            ...
            try {
                mAdapter = new KLSimpleTreeAdapter<KLBean>(tx,mTree, this, mDatas, 10);
    ...
    private class QueryListener implements OnQueryTextListener {
        ...
        public boolean onQueryTextChange(String newText) {
                ...
                try {
                    KLTreeListViewAdapter chageadapter= new KLSimpleTreeAdapter<KLBean>(tx,mTree, getApplicationContext(), mDatas, 10);
